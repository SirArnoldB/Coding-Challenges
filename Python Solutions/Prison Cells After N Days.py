...
class Solution:
    def nextStep(self, cells):
        """Return the next cells."""
        step = [0] * 8
        for i in range(1, 7):
            step[i] = int(cells[i-1] == cells[i+1])
        return step
    ...
    def prisonAfterNDays(self,cells, n):
        ...
        seen_cells = set()
        for size in range(n):
            next_cells = self.nextStep(cells)
            next_cells_str = str(next_cells)
            if next_cells_str not in seen_cells:
                seen_cells.add(next_cells_str)
            else:
